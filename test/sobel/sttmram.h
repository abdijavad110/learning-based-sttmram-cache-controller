...
void we_setup(char* ql_config_str, int seed, bool randomCacheInit){
  ...
  for(int i=0; i<num_ql_configs; i++)
    printf("%d - %d \n", i, ql_config[i]);
#endif
...
//  printf("> block size meanwhile is %d\n", blockSize);
//  printf(">>>> %u, %u, %u, %u\n", blockBaseAddr, blockSize, arrayBaseAddr, arraySizeByte);
