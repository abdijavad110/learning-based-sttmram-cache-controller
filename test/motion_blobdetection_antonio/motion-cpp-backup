...
double blob_DtoE = 0.001;
double blob_CtoD = 0.00001;
double blob_FtoG = 0.0001;
double blob_HtoI = 0.001;

double erosion_A = 0.001;
double erosion_C = 0.001;

//// golden
//double ber[8]={0,0,0,0,0,0,0,0}; // golden
//double blob_HtoI = 0.0000001;
//double blob_DtoE = 0.0000001;
//double blob_CtoD = 0.0000001;
//double blob_FtoG = 0.0000001;
//AMHM End
...
int main(int argc, char **argv) {
  ...
  /// JA codes start
  sprintf(var_buf_s, "%s,%lld\n", "bgImage", (long long int) &(bgImage.data[0]));
  strcat(var_buf_l, var_buf_s);
  sprintf(var_buf_s, "%s,%lld\n", "originalImage", (long long int) &(originalImage.data[0]));
  strcat(var_buf_l, var_buf_s);
  sprintf(var_buf_s, "%s,%lld\n", "bgGreyImage", (long long int) &(bgGreyImage.data[0]));
  strcat(var_buf_l, var_buf_s);
  sprintf(var_buf_s, "%s,%lld\n", "greyImage", (long long int) &(greyImage.data[0]));
  strcat(var_buf_l, var_buf_s);
  sprintf(var_buf_s, "%s,%lld\n", "motionImage", (long long int) &(motionImage.data[0]));
  strcat(var_buf_l, var_buf_s);
  sprintf(var_buf_s, "%s,%lld\n", "blobImage", (long long int) &(blobImage.data[0]));
  strcat(var_buf_l, var_buf_s);
  sprintf(var_buf_s, "%s,%lld\n", "erosionImage", (long long int) &(erosionImage.data[0]));
  strcat(var_buf_l, var_buf_s);
  sprintf(var_buf_s, "%s,%lld\n", "outputImage", (long long int) &(outputImage.data[0]));
  strcat(var_buf_l, var_buf_s);
  sprintf(var_buf_s, "%s,%lld\n", "outputImage2", (long long int) &(outputImage2.data[0]));
  strcat(var_buf_l, var_buf_s);
  /// JA codes end
...
//  printf("bgImage to bgGreyImage (rgb2grey) START time: %f\n", TIME);
  ...
  sprintf(exec_buf_s, "%s,%f,%f\n", "bgImage to bgGreyImage (rgb2grey)", time_latch, TIME);
  strcat(exec_buf_l, exec_buf_s);    /// JA code
...
//  printf("originalImage to greyImage (rgb2grey) START time: %f\n", (double)(clock()/CLOCKS_PER_SEC));
  ...
  sprintf(exec_buf_s, "%s,%f,%f\n", "originalImage to greyImage (rgb2grey)", time_latch, TIME);
  strcat(exec_buf_l, exec_buf_s);    /// JA code
...
//  printf("motion detection START time: %f\n", (double)(clock()/CLOCKS_PER_SEC));
  ...
  sprintf(exec_buf_s, "%s,%f,%f\n", "motion detection", time_latch, TIME);
  strcat(exec_buf_l, exec_buf_s);    /// JA code
//  printf("motion detection END time: %f\n", (double)(clock()/CLOCKS_PER_SEC));
  ...
  AMHM_qual(ToUnsignedInt(erosion_A));
  printf("-----------------------------------------------------------------\n");
...
//  printf("erosion filter START time: %f\n", (double)(clock()/CLOCKS_PER_SEC));
  ...
  sprintf(exec_buf_s, "%s,%f,%f\n", "erosion filter", time_latch, TIME);
  strcat(exec_buf_l, exec_buf_s);    /// JA code
...
//  printf("blob detection START time: %f\n", double(clock())/CLOCKS_PER_SEC);
  time_latch = TIME;
  AMHM_approx((long long int)&(erosionImage.data[0]), (long long int) (&erosionImage.data[0] + sizeof(unsigned char)*erosionImage.rows*erosionImage.cols*1 - 1));
  AMHM_qual(ToUnsignedInt(ber[EROSION_IMAGE]));
  bb_coords = blob_detection(&erosionImage, &blobImage, &bbs_num, output);
  sprintf(exec_buf_s, "%s,%f,%f\n", "blob detection", time_latch, TIME);
  strcat(exec_buf_l, exec_buf_s);    /// JA code
//  printf("blob detection END time: %f\n", double(clock())/CLOCKS_PER_SEC);
  ...
  if (output) {
    ...
    AMHM_approx((long long int)&(erosionImage.data[0]), (long long int) (&erosionImage.data[0] + sizeof(unsigned char)*erosionImage.rows*erosionImage.cols*1 - 1));
    AMHM_qual(ToUnsignedInt(erosion_C));
    grey2rgb(&outputImage2, &erosionImage);
    writeImage("output5.bmp", &outputImage2);
    printf(">>>>>>>>>>>>>>>> blob_F: %f\n", TIME);
    AMHM_approx((long long int)&(blobImage.data[0]), (long long int) (&blobImage.data[0] + sizeof(unsigned char)*blobImage.rows*blobImage.cols*1 - 1));
    AMHM_qual(ToUnsignedInt(blob_FtoG));
    grey2rgb(&outputImage2, &blobImage);
    printf(">>>>>>>>>>>>>>>> blob_G: %f\n", TIME);
    writeImage("output6.bmp", &outputImage2);
    printf(">>>>>>>>>>>>>>>> blob_H: %f\n", TIME);
    AMHM_approx((long long int)&(blobImage.data[0]), (long long int) (&blobImage.data[0] + sizeof(unsigned char)*blobImage.rows*blobImage.cols*1 - 1));
    AMHM_qual(ToUnsignedInt(blob_HtoI));
...
void motion_detection(sImage *originalImage, sImage *backgroundImage, sImage *outputImage) {
  ...
  for (r = 0; r < originalImage->rows; r++) {
    for (c = 0; c < originalImage->cols; c++) {
      outputImage->data[r * originalImage->cols + c] = (abs(
              originalImage->data[r * originalImage->cols + c] - backgroundImage->data[r * originalImage->cols + c]) >
                                                        MD_THRESHOLD) ? FG : BG;
    }
  }
}
...
void erosion_filter(sImage *originalImage, sImage *outputImage) {
  ...
  for (r = 0; r < originalImage->rows; r++) {
    for (c = 0; c < originalImage->cols; c++) {
      marker = 0;
      count = 0;
      for (r1 = (r - (EROSION_MAP_SIZE - 1) / 2 > 0) ? (r - (EROSION_MAP_SIZE - 1) / 2) : 0;
           r1 < r + (EROSION_MAP_SIZE - 1) / 2 && r1 < originalImage->rows; r1++) {
        for (c1 = (c - (EROSION_MAP_SIZE - 1) / 2 > 0) ? c - (EROSION_MAP_SIZE - 1) / 2 : 0;
             c1 < c + (EROSION_MAP_SIZE - 1) / 2 && c1 < originalImage->cols; c1++) {
          count++;
...
int **blob_detection(sImage *originalImage, sImage *blobImage, int *num_of_bbs, bool output) {
  ...
  printf(">>>>>>>>>>>>>>>> blob_C: %f\n", TIME);
  AMHM_approx((long long int)&(blobImage->data[0]), (long long int) (&blobImage->data[0] + sizeof(unsigned char)*blobImage->rows*blobImage->cols*1 - 1));
  AMHM_qual(ToUnsignedInt(blob_CtoD));
  ...
  printf(">>>>>>>>>>>>>>>> blob_D: %f\n", TIME);
  AMHM_approx((long long int)&(blobImage->data[0]), (long long int) (&blobImage->data[0] + sizeof(unsigned char)*blobImage->rows*blobImage->cols*1 - 1));
  AMHM_qual(ToUnsignedInt(blob_DtoE));
  if (output)
    for (r = 0; r < blobImage->rows; r++)
      for (c = 0; c < blobImage->cols; c++) {
