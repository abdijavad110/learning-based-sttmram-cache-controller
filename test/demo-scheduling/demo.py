...
class DemoScheduler:
  ...
  def periodic(self, time, time_delta):
    if time_delta:
      for core in range(sim.config.ncores):
        icount = sim.mpd.get('performance_model', core, 'instruction_count')
  ...
  def print_info(self):
    ...
    print '   idle:',
    for core in range(sim.config.ncores):
      print '%2.0f%%' % (100 * sim.mpd.get('performance_model', core, 'idle_elapsed_time') / float(sim.mpd.time())),
    print '   threads:',
    for thread in range(sim.thread.get_nthreads()):
      print '%5dkins' % (sim.mpd.get('thread', thread, 'instruction_count') / 1e3),
    print

  def hook_roi_end(self):
    print
    print '[DEMO] Total runtime = %d us' % (sim.mpd.time() / 1e9)
    print

  def reschedule(self):
    if False:
      threads = [(thread, sim.mpd.get('thread', thread, 'instruction_count')) for thread in range(sim.thread.get_nthreads())]
